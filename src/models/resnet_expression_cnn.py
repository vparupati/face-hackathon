...
class ResNetExpression(nn.Module):
    """ResNet18 adapted for 48x48 grayscale expression recognition"""
    
    def __init__(self, num_classes=7, input_channels=1):
        super(ResNetExpression, self).__init__()
        
        # Load ResNet18 (no pretrained weights to avoid hash issues)
    ...
    def forward(self, x):
        # Process grayscale input directly
...
class ImprovedExpressionTrainer:
    ...
    def __init__(self, model, device, num_classes=7, learning_rate=1e-3, weight_decay=1e-4):
        ...
        self.criterion = FocalLoss(alpha=None, gamma=2)
...
def get_advanced_transforms(img_size=100, is_grayscale=False):
    """Advanced data augmentation for facial expression images"""
    
    if is_grayscale:
        # For FER2013 (48x48 grayscale)
...
def train_resnet_model(data_dir: str, out_path: str, epochs: int = 50, img_size: int = 100, 
                      batch: int = 64, lr: float = 1e-3, model_type: str = "resnet", is_grayscale: bool = False):
    """Train ResNet expression model with M4 Max optimizations"""
    ...
    # Advanced transforms
    train_transform, val_transform = get_advanced_transforms(img_size, is_grayscale)
    ...
    if model_type == "resnet":
        ...
        model = ResNetExpression(num_classes=num_classes, input_channels=input_channels)
        print(f"Using ResNet18 with {sum(p.numel() for p in model.parameters() if p.requires_grad):,} parameters")
        print(f"Input: {input_channels} channels, {img_size}x{img_size} resolution")
    else:
...
def main():
    parser = argparse.ArgumentParser(description="Train ResNet expression model")
